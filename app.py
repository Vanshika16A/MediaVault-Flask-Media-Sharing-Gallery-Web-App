...
connection = psycopg2.connect("postgresql://kayak:GqQdCGtiNrX9sy1Xoex_lA@kayak123-4080.7s5.aws-ap-south-1.cockroachlabs.cloud:26257/project?sslmode=require")
print("connection established")


app = Flask(__name__)
# connection = pymysql.connect(host='@kayak123-4080.7s5.aws-ap-south-1.cockroachlabs.cloud',
#                              port=26257,
#                              user='kayak',
#                              password='GqQdCGtiNrX9sy1Xoex_lA',
#                              database='Project',
...
def audio_index():
        ...
        with connection.cursor() as cursor:
            cursor.execute("SELECT audio_blob, audio_name FROM Audio WHERE user_id = %s", (user_id))
            results = cursor.fetchall()
        for result in results:
            audio_data = result['audio_blob']
...
def fetch_audio_from_database(audio_id):
    cursor2.execute("SELECT audio_data FROM Audio WHERE audio_id = %s", (audio_id,))
    audio_data = cursor2.fetchone()[0]
...
#     return base64_string + b'=' * padding_needed
...
def display_images2():
    ...
    for result in results:
        ...
        image_name = result[1]
        images.append({
            'src': f"data:image/{image_extension};base64,{image_data.decode()}",
            'alt': image_name
        })
...
def search_images():
    ...
    user_id = session.get('user_id')
    if user_id is None:
        return 'Unauthorized', 401  # Hardcoded for now
    images = []
    with connection.cursor() as cursor:
        ...
        results = cursor.fetchall()
    for result in results:
        image_data = result['image_blob']
        image_extension = result['image_extension']
        image_name = result['image_name']
        images.append({
            'src': f"data:image/{image_extension};base64,{image_data.decode('utf-8')}",
            'alt': image_name,
        })
...
def display_images():
    ...
    return render_template('dummy.html', images=images)
...
def retrieve_image_data(user_id, video_quality="720p"):
    ...
    cursor2.execute("SELECT image_blob, image_duration FROM Images WHERE user_id = %s AND image_selected=1", (user_id,))
    image_records = cursor2.fetchall()
    cursor2.execute("SELECT audio_id FROM Images WHERE user_id = %s AND image_selected=1", (user_id,))
    audio_records = cursor2.fetchall()
    print(audio_records)
    cursor2.execute("SELECT image_transition FROM Images WHERE user_id = %s AND image_selected=1", (user_id,))
    transitions = cursor2.fetchall()
    print(transitions)
...
def create_video(image_records, audio_records, video_quality, transitions): 
    ...
    # final_output_path = './static/final_output_video.mp4'
...
def generate_video():
    ...
    video_clip = create_video(image_records, audio_records, video_quality, transitions)
